...
public:
  ...
  template <typename T>
  static T get(const std::string& parameterName, const T& defaultValue)
  {
    assert(s_instance);

    // Just a loose wrapper around the ROS parameter server API, uses local caching to avoid delays by having to contact
    // the ROS master
    T value;
  ...
  template <typename T>
  static T set(const std::string& parameterName, const T& value)
  {
    assert(s_instance);
