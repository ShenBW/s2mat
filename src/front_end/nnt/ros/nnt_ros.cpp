...
namespace nnt
{
ROSInterface::ROSInterface(ros::NodeHandle& nodeHandle)
  : m_nodeHandle(nodeHandle), m_params(nodeHandle), m_geometryUtils(), m_tracker(NULL), m_timingInitialized(false)
{
  ...
  const std::string cluster_topic = "/clusters";

  m_detectedPersonsSubscriber = m_nodeHandle.subscribe<jsk_recognition_msgs::BoundingBoxArray>(
      cluster_topic, queue_size, boost::bind(&ROSInterface::incomingObservations, this, _1));
}
...
void ROSInterface::incomingObservations(const jsk_recognition_msgs::BoundingBoxArray::ConstPtr clusters_msg)
{
